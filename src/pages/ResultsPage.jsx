...
const handleGetUserById = async (e) => {
  ...
  setLoading(true);
  setError('');
  setCustomers([]); // Clear out customers list to prevent table overlap

  try {
    const baseUrl = import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:5000';
    
    // Switch the fetch to target either your user-to-account lookup or account lookup endpoint
    const response = await fetch(`${baseUrl}/api/accounts/${searchId.trim()}`);

    if (response.status === 404) {
      throw new Error(`Account information for ID "${searchId}" not found.`);
    }
    ...
    // Normalize single objects into an array so the map function doesn't crash
    const normalizedData = Array.isArray(data) ? data : [data];
    
    setAccounts(normalizedData); // Set the accounts state!
  } catch (err) {
    setAccounts([]);
